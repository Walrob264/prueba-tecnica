...
export default function Pagination({ pag, setPag, max }) {
  ...
  const getVisiblePages = () => {
    ...
    while (visiblePagesArray.length > 4) {
      if (visiblePagesArray[0] !== pag && (visiblePagesArray[0] !== prevPag || pag === 1)) {
        visiblePagesArray.shift();
      } else if (visiblePagesArray[visiblePagesArray.length - 1] !== pag && (visiblePagesArray[visiblePagesArray.length - 1] !== prevPag || pag === max)) {
  ...
  return (
    <>
      <div
        style={{ width: "100%", display: "flex", justifyContent: "center", gap: "0.5em" }}
